...
#### (Non-)Participation in the Type System

A key design decision for a compile-time computation system is whether or not compile-time values may affect types of program values. This proposal for Swift’s compile-time programming capabilities focuses on the knowability and computability of compile-time values and intentionally prevents such values from having an effect on the type system. As such, compile-time knowability is a property of values (variables, parameters, etc.) and computation (expressions, functions, etc.) thereof, but never the corresponding type. This means that compile-time programming can benefit from Swift’s type safety without introducing additional complexity into the type system and allows a seamless interoperability of compile-time source code with runtime code which does not require any kind of implicit or explicit conversion. For example, compile-time functions behave as “normal” runtime functions when invoked with runtime-known-value arguments and vice versa: not-explicitly-marked-as-compile-time functions may be called from compile-time expressions if the compiler is able to infer them as compile-time evaluable.

Furthermore, it is the goal of this proposal to lay the groundwork to allow for an expansion of the language surface which is able to participate in compile-time computation over time; and as such, participation in the type system would introduce further capabilities for general computation into type-checking, overload resolution, and related systems, which is undesirable for Swift, which already has a powerful and expensive type resolution system. With this design principle established, SIL provides a natural surface for engineering a general-purpose compile-time evaluation over a subset of the Swift language.
